...
class Excel:
    ...
    def bilanco(self):
        ...
        try:
            ...
            raise ValueError(os.path.basename(self.path))
    ...
    def bilanco_calcualtions(self,df,saveName):
        ...
        try:
            ...
            df['Bakiye'] = df[['Borç Bakiye', 'Alacak Bakiye']].apply(lambda x:x[0]-x[1],axis=1) # calculate the Bakiye column
            self.deleteColumns(['Detay', 'Borç Toplamı', 'Alacak Toplamı', 'Borç Bakiye','Alacak Bakiye'], df) # delete the unneeded columns
            df = df[notnull(df["Hesap Kodu"])]
            self.codes_balances = dict(zip(df['Hesap Kodu'].apply(lambda x:int(x)), df['Bakiye'])) # get codes with their values from Bakiye
    ...
    def loadFiles(self):
        ...
        for file in self.files:
            ...
            try:
                ...
                self.deleteColumns(['Stok Kodu','Sıralama','Belge Türü','Belge Türü Açıklaması','Masraf Merkezi','COLUMN1','Yevmiye No','Fiş Türü'],df)

            except (ValueError,AssertionError):
                print ('in')
                print (os.path.basename(file)) 
                raise ValueError(str(os.path.basename(file)))

            except KeyError:
                open_workbook.close()
                raise KeyError(str(os.path.basename(file)))

            except Exception:
                raise Exception('Not Expected Error '+' '+self.loadFiles.__qualname__)
