...
class GUI(Frame):
    ...
    def Run(self,opType):
        ...
        if ("." in saveName): # prevent the dots in the save name
            ...
            if opType == 1:
                ...
                if (pattern.search(self.balance_date.get())==None):
    ...
    def calcualte(self,saveName,opType):
        ...
        try:
            ...
            self.error_state()
            Msg.showwarning('Notice',"The file"+str(E)+"does not follow the accepted format.")
            ...
            Msg.showwarning('Error','Error on line {}'.format(sys.exc_info()[-1].tb_lineno)+" "+type(E).__name__+"\n"+
                            "Please send the input files with a screen shot of this window to get help.")
        finally:
